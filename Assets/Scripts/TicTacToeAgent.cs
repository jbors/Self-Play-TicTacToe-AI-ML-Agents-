...
public class TicTacToeAgent : Agent
{
    ...
    private int EvaluateMove(Field[] boardState, int move, Player player)
    {
        ...
        if (state == WinState.CrossWin && player == Player.X)
        {
            return 1;
        } else if (state == WinState.NoughtWin && player == Player.O)
        {
            return 1;
        } else if (state == WinState.NoughtWin && player == Player.X)
        {
            return -1;
        } else if (state == WinState.CrossWin && player == Player.O)
        {
            return -1;
        }
